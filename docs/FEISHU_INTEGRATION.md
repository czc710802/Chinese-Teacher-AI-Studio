...
1. 批改完成总览卡：学生、班级、题目、字数、总分、等级、总评、优点、问题、修改方向。
2. 分页详情卡：总评、审题立意、内容结构、逻辑论证、语言表达、逐段点评、错别字病句、修改方案、示范修改。
3. 完整网页报告：统一跳转到 `https://pi.zhenwanyue.icu/report/:archiveId?...`，避免在飞书里截断长文本。
...
当 `FEISHU_FILE_UPLOAD_ENABLED=true` 且飞书应用具备文件上传权限时，服务端会先上传 PDF / DOCX，再把文件消息发回当前会话；上传失败会自动回退到签名链接，不影响主批改流程。
...
教师审核后可通过 `POST /api/essays/:id/publish-report` 将报告状态标记为已发布。若学生已绑定飞书身份，系统会发送摘要卡片；完整报告和 PDF 下载使用公网链接，不把长报告正文直接塞进飞书消息。分页卡按钮会携带 `archiveId` 和 `page`，点击后由飞书服务端重新渲染对应页，不依赖重新提交作文。
