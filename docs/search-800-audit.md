...
## 2026-07-16 补充：1000 字限制复查

本次复查确认：

1. 运行时代码中没有发现 `wordCount < 1000`、`content.length < 1000` 或“最高限制1000字”的硬拦截。
2. 1000 相关位置只剩下长文分档提示和测试断言，不再承担提交拒绝职责。
3. 构建产物 `client/dist` 需要随源码一起重新 build，避免旧 bundle 继续把旧提示带到线上。
4. 若线上仍出现“上传批改失败：作文字符数超出，最高限制1000字”，应优先核查部署实例是否仍在读取旧构建产物或旧服务进程，而不是继续在源码里寻找已不存在的硬拦截。
